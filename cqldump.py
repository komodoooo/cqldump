#!/bin/env python3
import argparse,subprocess,select,os
...
for keyspace_row in keyspaces:
    ...
    for table_row in tables:
        ...
        try:
            session.execute(f"SELECT * FROM {keyspace_name}.{table_name} LIMIT 1;")
        except:
            continue
...
os.mkdir(name)
os.chdir(name)
cqlsh_process = subprocess.Popen(["cqlsh", args.host], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
for table in cluster_tables:
    ...
    exported:bool=False
    while not exported:
        text=select.select([cqlsh_process.stdout, cqlsh_process.stderr], [], [], 0.1)[0]
        for strm in text:
            output = strm.readline()            # all this only to sync a bit the output
            if "Processed: " in output:
                exported=True
                break
    print(f"Successfully exported {table} to {name}/{table}.csv")
